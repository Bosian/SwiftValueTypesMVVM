...
final public class EVReflection {
    ...
    */
     ...
     */
    public class func toJsonString(_ theObject: NSObject, performKeyCleanup:Bool = true) -> String {
    ...
    public static func setObjectValue<T where T:NSObject>(_ anyObject: T, key:String, value:AnyObject?, typeInObject:String? = nil) {
        ...
        if value == nil || value as? NSNull != nil {
            ...
            if (typeInObject == "String" || typeInObject == "NSString") && type == "NSNumber" {
                ...
                if let convertedValue = value as? String {
                    value = getDateFormatter().date(from: convertedValue)
                    if value == nil {
        ...
        if let type = fieldType {
            if type.hasPrefix("Array<") && dictValue as? NSDictionary != nil {
                if (dictValue as! NSDictionary).count == 1 {
                    // XMLDictionary fix
                    if let i = (dictValue as! NSDictionary).makeIterator().next()!.value as? NSArray {
                        dictValue = i
